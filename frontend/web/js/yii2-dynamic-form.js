    ...
    var _restoreSpecialJs = function(widgetOptions) {
        ...
        if ($hasMaskmoney.length > 0) {
            $hasMaskmoney.each(function() {
                ...
                console.log(eval($(this).attr('data-krajee-maskmoney')));
                $(displayID).maskMoney('mask', parseFloat($(id).val()));
                $(displayID).on('change keyup', function (e) {
                    if (e.type ==='change' || (e.type === 'keyup' && (e.keyCode == 13 || e.which == 13))) {
                        var out = $(displayID).maskMoney('unmasked')[0];
                        $(id).val(out).trigger('change');
                    }
                });
            });
        }
